...
def filter_df_2(df,educational_area="AI Programmer"):
    return df.query("Utbildningsnamn == @educational_area")["Utbildningsnamn"].value_counts().reset_index().rename({"count": "Ansökta utbildningar"}, axis=1)
...
selected_graph = "Kommun"

if selected_graph == "Kommun":
    df_municipality = filter_df(df)
    municipality_chart = create_municipality_bar(
        df_municipality.head(10), y="Kommun", x="Ansökta utbildningar",ylable= "KOMMUN", xlable="# ANSÖKTA UTBILDNINGAR")
...
elif selected_graph != "Kommun":
    df_municipality = filter_df_2(df)
    municipality_chart = create_municipality_bar(
        df_municipality.head(10),y="Utbildningsnamn", x="Ansökta utbildningar",ylable= "KOMMUN", xlable="# ANSÖKTA UTBILDNINGAR")

    number_municipalites = 5
    municipalites_title = number_municipalites

    selected_educational_area = "AI Programmer"
    educational_area_title = selected_educational_area
    ...
    with tgb.part(class_name="container stack card"):
        ...
        with tgb.layout(columns="1 2"):
            ...
            with tgb.part(class_name="card") as column_chart:
                ...
                tgb.selector(value="{selected_graph}",
                             lov=["Kommun", "Utbildning"],
                             dropdown=True)
